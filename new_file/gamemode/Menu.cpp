	...
	namespace gamemode
	{
		#define PP_upDown() \
			switch( keyList.getCursor() ) { \
				case KEY_DOWN: if( incCursor() ) getOwner().getAudio2D().playSound(getOwner().getProject().getLDB()[22].getArray1D()[41]); break; \
				case KEY_UP  : if( decCursor() ) getOwner().getAudio2D().playSound(getOwner().getProject().getLDB()[22].getArray1D()[41]); break; \
				default: break; \
			}

		Equip::Equip(rpg2kLib::Main& m)
		: GameMode(m)
		{
		...
		// equipment name
			for(int i = 0; i < EQ_END; i++) equip_.push_back( voc[0x88 + i] );
		}
		...
		void Equip::run(input::KeyListener& keyList)
		{
			...
			switch( cursorNum() ) {
				case 2: // select equip type
					...
					PP_upDown()
					switch( keyList.getCursor() ) {
						case KEY_RIGHT: incCursor(0); break;
						case KEY_LEFT : decCursor(0); break;
						default: break;
					}
					break;
				case 3: // select equiping item
					break;
					if( keyList.enter() ) {
						proj.unequip( lsd.member( cursor(0) ),  (EquipType)cursor(1) );
						proj.  equip( lsd.member( cursor(0) ),  idList_[ cursor(2) ] );
						removeLastCursor();
					}
					switch( keyList.getCursor() ) {
						case KEY_UP   : decCursor(); cursor()--; break;
						case KEY_RIGHT: incCursor(); break;
						case KEY_LEFT : decCursor(); break;
						case KEY_DOWN : incCursor(); cursor()++; break;
						default: break;
					}
					break;
			}
		...
		void Skill::draw(Graphics2D& g)
		{
			...
			// model::SaveData& lsd = proj.getLSD();
			model::DataBase& ldb = proj.getLDB();
			structure::Array1D const& voc = ldb[21];
			std::ostringstream ss;
			...
			switch( cursorNum() ) {
				case 2: { // select skill
					...
					g.drawString( voc[128], base, font::FNT_STATUS );
					...
					g.drawString( voc[129], base, font::FNT_STATUS );
					base += Vector2D( font::FULL_FONT_W*1, 0 );
					ss.str(""); ss << std::setw(3) << proj.hp(curCharID_) << "/" << std::setw(3) << proj.param(curCharID_, PRM_HP);
					g.drawString( ss.str(), base );
					// MP
					base += Vector2D( font::HALF_FONT_W*9, 0 );
					g.drawString( voc[130], base, font::FNT_STATUS );
					base += Vector2D( font::FULL_FONT_W*1, 0 );
					ss.str(""); ss << std::setw(3) << proj.mp(curCharID_) << "/" << std::setw(3) << proj.param(curCharID_, PRM_MP);
					g.drawString( ss.str(), base );
					...
					for(std::vector< uint16_t >::const_iterator it = idList_.begin(); it != idList_.end(); ++it) {
						...
						ss.str(""); ss << "-" << std::setw(3) << skill[*it][11].get_int();
		...
		void Skill::run(input::KeyListener& keyList)
		{
			...
			switch( cursorNum() ) {
				case 2: // select skill
					if( keyList.cancel() ) removeLastCursor(); // skip return to source char select
					switch( keyList.getCursor() ) {
						case KEY_UP   :  --cursor(); decCursor(); break;
						case KEY_RIGHT: incCursor(); break;
						case KEY_LEFT : decCursor(); break;
						case KEY_DOWN :  ++cursor(); incCursor(); break;
						default: break;
					}
					break;
		...
		// char info
			...
			for(uint i = 0; i < mem.size(); i++) {
				...
				// HP
				g.drawString( voc[129], base + Vector2D(font::HALF_FONT_W*18, 0), font::FNT_STATUS );
				ss.str(""); ss << std::setw(3) << proj.hp(charID) << "/" << std::setw(3) << proj.param(charID, PRM_HP);
				g.drawString( ss.str(), base + Vector2D(font::HALF_FONT_W*20, 0) );

				base += Vector2D(0, font::FONT_H + 3);
				// experience
				g.drawString(voc[127], base, font::FNT_STATUS);
				ss.str("");
				...
				g.drawString( ss.str(), base + Vector2D(font::FULL_FONT_W, 0) );
				// MP
				g.drawString( voc[130], base + Vector2D(font::HALF_FONT_W*18, 0), font::FNT_STATUS );
				ss.str(""); ss << std::setw(3) << proj.mp(charID) << "/" << std::setw(3) << proj.param(charID, PRM_MP);
				g.drawString( ss.str(), base + Vector2D(font::HALF_FONT_W*20, 0) );
			}
		}
