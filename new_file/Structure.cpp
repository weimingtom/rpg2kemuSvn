...
namespace rpg2kLib
{
	...
	namespace structure
	{
		...
		int Binary::toNumber() const
		{
			...
			if( getBERSize(ret) == size() ) return ret;
				...
				cerr << "Binary::getNumber(): This Binary isn't BER number. "
					...
					<< ";" << endl;
				throw std::logic_error("Failed converting Binary to number.");
			}
		}
		bool Binary::toBool() const
		{
			...
			throw std::logic_error("Failed converting Binary to bool.");
		}
		double Binary::toDouble() const
		{
			if( size() != sizeof(double) ) {
				cerr << "Binary::getDouble(): This Binary isn't double. "
					<< "size() = " << size() << ";" << endl;
				throw std::logic_error("Failed converting Binary to double.");
		...
		void Binary::exchangeEndianIfNeed(uint8_t* dst, const uint8_t* src, uint sizeOf, uint eleNum)
		{
			if(eleNum == 0) return;

			#if RPG2K_IS_BIG_ENDIAN
				for(uint j = 0; j < eleNum; j++) {
					const uint8_t* srcCur = src + sizeOf*j
