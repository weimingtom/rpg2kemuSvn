	...
	namespace gamemode
	{
		...
		// mapping non-events
			for(uint i = EV_ID_PARTY; i <= EV_ID_AIRSHIP; i++) {
				...
				if( eventMap_[EV_PR_CHAR].find(x) == eventMap_[EV_PR_CHAR].end() ) {
					eventMap_[EV_PR_CHAR].insert( y_val( y, std::multimap< uint, uint >() ) );
				}
				eventMap_[EV_PR_CHAR][y].insert( x_val(x, i) );

				if(i == EV_ID_PARTY) {
					RPG2kKey cursor = keyList.getCursor();
					if( (cursor != KEY_NONE) && judgeMove(i, toEventDir(cursor) ) ) {
						// do something
					}
				}
			}
		// process event
			...
			for(int step = 0; step < EV_STEP_MAX; step++) {
				...
				if(inst == NULL) {
					...
					case model::EXE_FAILED:
						break;
					case model::EXE_SUCCESS:
						break;
					case model::EXE_NOT_IMPLEMENTED:
						break;
					case model::EXE_NEXT_FRAME: return;
					default:
		...
		// horizon
			if( !(scrll&0x02) ) {
				if( lmu[2].get_uint() <= (baseP[0] + CHIP_NUM[0]) )
					baseP[0] = lmu[2].get_uint() - CHIP_NUM[0];
				else if(baseP[0] < 0) baseP[0] = 0;
			}
		// vertical
			if( !(scrll&0x01) ) {
				if( lmu[3].get_uint() <= (baseP[1] + CHIP_NUM[1]) )
					baseP[1] = lmu[3].get_uint() - CHIP_NUM[1];
		...
		void Field::drawPictures(Graphics2D& g)
		{
			...
			for(structure::Array2D::Iterator it = pictures.begin(); it != pictures.end(); ++it) {
				...
				Vector2D coord[4];
				Vector2D center( target[4].get_double(), target[5].get_double() );
				Image const& img = g.getPicture(target[1], target[9]);
				Size2D size = img.size() * target[33].get_int() / 100;

				switch( target[15].get_int() ) {
					default:
				...
				ColorRGBA tone;
				tone[3] = ( 100 - target[34].get_int() ) / 100.0f;
				for(int i = 0; i < 3; i++)
					tone[i] = target[11 + i].get_double() * target[14].get_double() / 100.0f / 100.0f;
		...
		static int nextX(EventDir dir, uint x)
		{
			switch(dir) {
				case EV_DIR_LEFT : return x-1;
				case EV_DIR_RIGHT: return x+1;
				case EV_DIR_DOWN : case EV_DIR_UP   : default: return x;
			}
		}
		static int nextY(EventDir dir, uint y)
		{
			switch(dir) {
				case EV_DIR_DOWN : return y+1;
				case EV_DIR_UP   : return y-1;
				case EV_DIR_LEFT : case EV_DIR_RIGHT: default: return y;
			}
		}

		bool Field::judgeMove(uint eventID, EventDir dir)
		{
		...
		// with map chip level
			...
			// with event level
				// priority
				uint pr;
				if( (EV_ID_PARTY <= eventID) && (eventID <= EV_ID_AIRSHIP) ) pr = EV_PR_CHAR;
				else pr = page[34];
				// char
				if( (pr == EV_PR_CHAR) && gamemode::exists(eventMap_[pr], nxtX, nxtY) ) {
		...
		bool Field::canPassMap(EventDir dir, int curX, int curY, int nxtX, int nxtY)
		{
			model::Project& proj = getOwner().getProject();
			model::MapUnit& lmu = proj.getLMU();
