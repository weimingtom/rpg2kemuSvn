#define RPG2K_ENABLE_EVENT_COMMAND
	...
	namespace model
	{
		...
		: owner_(m)
		{
			#define PP_addEventCommand(code) \
				commandTable_.insert( \
					std::map< uint, CommandInterface >::value_type( \
						code, Command< code >(*this) \
					) \
				)
		...
		// get page number and map map events
			...
			for(structure::Array2D::Iterator it = mapEv.begin(); it != mapEv.end(); ++it) {
				...
				ExecInfo info = {
					EV_MAP, eventID,
					0, false,
					(EventStart) (int)page[33], pageID, &( page[52].getEvent() )
				};

				execInfo_[EV_MAP].insert( std::map< int, ExecInfo >::value_type(eventID, info) );
			}
		// map common events
			...
			for(structure::Array2D::Iterator it = comEv.begin(); it != comEv.end(); ++it) {
				...
				if(
					...
					ExecInfo info = {
						EV_MAP, eventID,
						0, false,
						(EventStart) (int)cur[11], 0, &( cur[22].getEvent() )
					};

					execInfo_[EV_COMMON].insert( std::map< int, ExecInfo >::value_type(eventID, info) );
				}
			}
			nextEvent();
		}

		ExecuteState Execute::execute(structure::Instruction const& inst)
		{
			if( commandTable_.find( inst.code() ) != commandTable_.end() ) {
				return commandTable_.find( inst.code() )->second( inst.getString(), inst.getArgs() );
			} else throw std::runtime_error("Undefined event command.");
		}
		ExecuteState Execute::executeBattle(structure::Instruction const& inst)
		{
			if( battleCommandTable_.find( inst.code() ) != battleCommandTable_.end() ) {
		...
		bool Execute::nextEvent()
		{
			...
			PP_iterate(maps, EV_AUTO)
			// parallel map event
			PP_iterate(maps, EV_PARALLEL)
			// auto start common event
			PP_iterate(coms, EV_AUTO)
			// parallel common event
			PP_iterate(coms, EV_PARALLEL)
			#undef PP_iterate
/*
 ...
 */
		...
		#define PP_codeDef(codeNo) \
			template<> ExecuteState Execute::Command< codeNo >::operator() \
			(RPG2kString const& strArg, std::vector< int32_t > const& args) const
		#define PP_defaultThrow() default: throw std::invalid_argument("Invalid argument")
		...
		// change
			...
			for(std::vector< uint16_t >::const_iterator it = target.begin(); it  != target.end(); ++it) {
				charDatas[*it][32] = charDatas[*it][32].get_int() + val;
		...
		// change
			...
			for(std::vector< uint16_t >::const_iterator it = target.begin(); it  != target.end(); ++it) {
				int tmp = charDatas[*it][32].get_int() + val;

				// check range
				if(tmp < LV_MIN) tmp = LV_MIN;
				else if(tmp > LV_MAX) tmp = LV_MAX;

				if( ( tmp != charDatas[*it][32].get_int() ) && args[5] ) {
					charDatas[*it][32] = tmp;
					return EXE_NOT_IMPLEMENTED;
				}
			}
		...
		PP_codeDef(10430) // change character parameter
		{
			...
			for( std::vector< uint16_t >::const_iterator it = target.begin(); it != target.end(); it++ ) {
				charDatas[*it][index] = charDatas[*it][index].get_int() + val;
			}
		...
		PP_codeDef(10450) // change equipment
		{
			...
			// unequip
				case 1: {
					uint start, end;
					switch(args[3]) {
						case EQ_WEAPON: case EQ_SIELD: case EQ_ARMOR: case EQ_HELMET: case EQ_OTHER:
							start = end = args[3]; break;
						case EQ_END: // all
							start = 0; end = EQ_END-1; break;
						PP_defaultThrow();
					}
					for(std::vector< uint16_t >::const_iterator it = target.begin(); it != target.end(); ++it) {
		...
		// set value
			...
			// check maximum
				if( result > proj.param(*it, PRM_HP) ) {
					result = e;
		...
		// set value
			for( std::vector< uint >::const_iterator it = charIDs.begin(); it != charIDs.end(); it++ ) {
				...
				int result = e.get_int() + val;
			// check maximum
				if( result > proj.param(*it, PRM_MP) ) result = e;
			// check minimum
		...
		PP_codeDef(10490) // full cure
		{
			...
			for( std::vector< uint >::const_iterator it = charIDs.begin(); it != charIDs.end(); it++ ) {
				charDatas[*it][71] = proj.param(*it, PRM_HP);
				charDatas[*it][72] = proj.param(*it, PRM_MP);
			}
		...
		PP_codeDef(10680) // change system graphic
		{
			...
			if( ldbSys[71].get_int() != args[0] ) lsdSys[22] = args[0];
			if( ldbSys[72].get_int() != args[1] ) lsdSys[23] = args[1];
		...
		PP_codeDef(10690) // change screen refresh
		{
			...
			if( ldbSys[ 61+args[0] ].get_int() != args[1]) {
	...
	/*
	 * junction event checks the rpg2kLib::Main::getExecute().currentInst()
	 ...
	 */
		...
		{
			if( getOwner().getProject().getLMU()[1].get_int() != args[0] ) {
		...
		{
			Project& proj = getOwner().getProject();
			if( proj.getLMT( proj.getCurrentMapID() )[44].get_int() != args[0] ) {
		...
		{
			getOwner().getProject().getLSD().replace( (ChipSetType)args[0], args[1], args[2] );
