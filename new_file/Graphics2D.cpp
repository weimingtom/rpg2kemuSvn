...
namespace rpg2kLib
{
	...
	: owner_(m)
	{
		...
		if(flagRet == -1) throw SDL_GetError();
		if( SDL_SetVideoMode(SCREEN_SIZE[0], SCREEN_SIZE[1], 32, SDL_OPENGL) == NULL ) throw SDL_GetError();
	// set matrix mode
	...
	Image const& Graphics2D::getPicture(RPG2kString const& name, bool trans)
	{
		if( !picturePool_[trans].exists(name) ) {
			picturePool_[trans].addReference( name, loadImage(Material::Picture, name, trans) );
		}
	...
	Image const& Graphics2D::getImage(Material::Type type, RPG2kString const& name)
	{
		if( !imagePool_[type].exists(name) ) {
			...
			switch(type) {
				...
				#undef PP_case
				default: throw "Invalid material type.";
			}

			imagePool_[type].addReference( name, loadImage(type, name, trans) );
		}
		return imagePool_[type][name];
	}

	Image& Graphics2D::loadImage(Material::Type type, RPG2kString const& name, bool trans) const
	{
		switch(type) {
			#define PP_case(type) case Material::type:
		...
		for(int i = 0; i < Material::RES_END; i++) {
			for(int j = 0; j < IMG_END; j++) {
				...
				Image& ret = *new Image(texID, width, height);
				return ret;
			}
		}
	...
	uint Graphics2D::drawChar(RPG2kString::const_iterator& it, Vector2D const& p, uint color)
	{
		...
		int width =
			(t == font::FULL) ? font::FULL_FONT_W :
			(t == font::HALF) ? font::HALF_FONT_W :
			0;
		...
		font::font_t* data =
			(t == font::FULL) ? font::Font::getFull( it, getOwner().getProject().fontType() ) :
			(t == font::HALF) ? font::Font::getHalf(it) :
			NULL;
	...
	void Graphics2D::drawWindowType(Vector2D const& dstP, Size2D const& dstS, WindowType type)
	{
		...
		switch( getOwner().getProject().wallpaperType() ) {
			...
			default: throw std::runtime_error("Unknown wallpaper type");
		}
 ...
 */
			...
			int num = lsd.getReplace(CHIP_LOWER, chipID -  5000);
			Vector2D srcP(
				(num%6) + (num<96 ? 0 : 6) + 12,
				(num/6) - (num<96 ? 0 : 16)
			);
			...
			int num = lsd.getReplace(CHIP_UPPER, chipID - 10000);
			Vector2D srcP(
	...
	{
		Vector2D srcP(
			state.charSetPos() % 4 * CHAR_PAT_END + state.charSetPat(),
			state.charSetPos() / 4 * CHAR_DIR_END + state.charSetDir()
		);
		drawImage(
	...
	// read header
		if(
			( fread(sign, 1, sizeof(sign), fp) != sizeof(sign) ) ||
			( memcmp(sign, SIGN, SIGN_SIZE) != 0 )
		) {
			fclose(fp);
			return NULL;
		}

		width  = fgetc(fp) | (fgetc(fp) << CHAR_BIT); // read uint16_t little endian
		height = fgetc(fp) | (fgetc(fp) << CHAR_BIT);
		channels = trans ? 4 : 3;
	// init zlib
		z_stream z;
		z.zalloc   = Z_NULL;
	...
	// decode palette and image data
		...
		uint8_t* loadBuff = new uint8_t[PALETTE_SIZE + width*height];
		int status = Z_OK;
		z.next_out = (Bytef*) loadBuff;
		z.avail_out = PALETTE_SIZE + width*height;
		while(true) {
			if(z.avail_in == 0) {
				z.next_in  = (Bytef*) inbuff;
	...
	// convert
		Palette* palette = (Palette*) loadBuff;
		uint8_t* data = loadBuff + PALETTE_SIZE;
