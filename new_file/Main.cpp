...
namespace rpg2kLib
{
	...
	// init counter
		...
		#endif
	// initialize SDL
		if( SDL_Init(SDL_INIT_EVERYTHING) == -1 ) throw SDL_GetError();
		else debug::addAtExitFunction(SDL_Quit);
	// config values
		config();
	// init Project
		project_ = new model::Project(gameDir_);
		cout << "Project initialized." << endl;
	// set window title
		SDL_WM_SetCaption( project_->getGameTitle().toSystem().c_str(), NULL);
	// init Graphics2D
		graphics2d_ = new Graphics2D(*this);
		cout << "Graphics2D initialized." << endl;
	// init Audio2D
		audio2d_ = new Audio2D(*this);
		cout << "Audio2D initialized." << endl;
	// init KeyListener
		keyListener_ = new input::KeyListener(*this);
	...
	// init GameModes
		#define PP_add2map(name) \
			gameMode_.addReference( GameMode::name, *new gamemode::name(*this) ); \
		...
		gotoTitle();
	// init Execute
		execute_ = new model::Execute(*this);
	}
	Main::~Main()
	{
		dispose();
	...
	void Main::config()
	{
		...
		for(std::list< RPG2kString >::const_iterator it = token.begin(); it != token.end(); ++it) {
			...
			throw "Error when loading setting Stream.";
		}
		if(preType != EXP_END) throw "Error when loading setting Stream.";
