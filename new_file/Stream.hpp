...
namespace rpg2kLib
{
	namespace structure
	{
		...
		public:
			...
			virtual SystemString const& name() const { throw std::logic_error("Unimplemented"); }
			...
			virtual uint write(const uint8_t* data, uint size) { throw std::logic_error("Unimplemented"); }
		...
		protected:
			...
			FileInterface(SystemString const& filename, const char* mode);
		public:
		...
		public:
			...
			uint write(const uint8_t* data, uint size);
		};
		 ...
		 */
			...
			bool isFile  () const { return implement_.isFile  (); }
			bool isBinary() const { return implement_.isBinary(); }
		};
