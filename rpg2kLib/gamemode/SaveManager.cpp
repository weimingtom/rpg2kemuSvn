...
namespace rpg2kLib
{
	namespace gamemode
	{
		SaveManager::SaveManager(rpg2kLib::Main& m)
		: GameMode(m)
		{
			structure::Array1D const& voc = getOwner().getProject().getLDB()[21];

			level_   = voc[128].get_string();
			hp_      = voc[129].get_string();
			prefix_  = voc[148].get_string();
